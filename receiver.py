...
class ReceiverApp():
    def __init__(self,channel,role):
        ...
        self.file_listbox = tk.Listbox(self.root, selectmode=tk.SINGLE, width=50, height=10)
        self.file_listbox.pack(pady=(40,30))

        button_frame = tk.Frame(self.root, bg='#74abdc')
        button_frame.pack()

        self.list_files_button = tk.Button(button_frame, text="Lister fichier", font=('helvetica', '10', 'bold'), command=self.list_files)
        self.list_files_button.pack(side=tk.LEFT, padx=20)
        if self.role!=3:
            self.select_file_button = tk.Button(button_frame, text="Selectionner fichier", font=('helvetica', '10', 'bold'), command=self.select_file)
            self.select_file_button.pack(side=tk.LEFT, padx=20)
            
        return_button = tk.Button(self.root, text="Retour", command=lambda: self.return_to_menu)
    ...
    def list_files(self):
        try:
            ...
            request = {"action": "List_file", "data": {'channel': self.channel}}
    ...
    def select_file(self):
        ...
        try:
            ...
            client_socket.connect((SERVER_IP, SERVER_PORT))
            print(selected_file)
            request={"action": "select_file", "data": {'channel': self.channel,"selected_file":selected_file}}
            client_socket.send(json.dumps(request).encode())
