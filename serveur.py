...
def create_channel(name, password, creator_username):
    ...
    try:
        ...
        cursor.execute('INSERT INTO channels (name, password) VALUES (?, ?)', (name, password))
        conn.commit()
...
def join_channel(name, password, username):
    conn = sqlite3.connect('server_database.db')
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM channels WHERE name=? AND password=?', (name,password))
    channel = cursor.fetchone()
    if channel:
        channel_id=channel[0]
    ...
    try:
        while True:
            ...
            if action == "login":
                ...
                file_path = os.path.join(f"./file_interface/{channel}", file_name)
                ...
                file_path = os.path.join(f"./file_interface/{data['channel']}", file_name)
                ...
                channel_name = data.get("channel_name")
                username = data.get("username")
                role = data.get("role", 0)  # Default role is user (0), admin is (1)
