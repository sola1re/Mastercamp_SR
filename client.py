...
class LoginApp:
    def __init__(self, root):
        ...
        self.logo_label = tk.Label(logo_frame, image=self.logo_photo, bg='#74abdc')
        ...
        self.username_label = tk.Label(root, text="Nom d'utilisateur", font=('helvetica', 15, 'bold'), bg='#74abdc', fg='#cb2e30')
        self.username_label.pack()
        self.username_entry = tk.Entry(root)
        self.username_entry.pack(pady=(0,18))

        self.password_label = tk.Label(root, text="Mot de passe", font=('helvetica', '15', 'bold'), bg='#74abdc', fg='#cb2e30')
        ...
        self.login_button = tk.Button(button_frame, text="Se connecter", font=('helvetica', '10', 'bold'), command=self.login)
        self.login_button.pack(side=tk.LEFT, padx=5)

        self.register_button = tk.Button(button_frame, text="S'inscrire", font=('helvetica', '10', 'bold'), command=self.register)
    ...
    def login(self):
        ...
        response = self.send_request("login", {"username": username, "password": hashed_password})
    ...
    def register(self):
        ...
        password = self.password_entry.get()
        hashed_password = hash_password(password)
        response = self.send_request("register", {"username": username, "password": hashed_password})
        if response["status"] == "success":
    ...
    def send_request(self, action, data):
        try:
            ...
            messagebox.showerror("Error", f"Erreur de connexion au serveur: {e}")
            return None
...
class JoinChannel:
    def __init__(self, username):
        ...
        channel_label = tk.Label(self.root, text="Nom du canal :", font=('helvetica', 15, 'bold'), bg='#74abdc', fg='#cb2e30')
        ...
        password_label = tk.Label(self.root, text="Mot de passe du canal :", font=('helvetica', 15, 'bold'), bg='#74abdc', fg='#cb2e30')
        ...
        join_button = tk.Button(button_frame, text="Rejoindre", command=self.join_channel, font=('helvetica', '10', 'bold'))
        join_button.pack(side=tk.LEFT, padx=5, pady=10)

        create_button = tk.Button(button_frame, text="Créer", command=self.create_channel, font=('helvetica', '10', 'bold'))
    ...
    def join_channel(self):
        ...
        password = self.password_entry.get()
        hashed_password = hash_password(password)
        response = self.send_request("join_channel", {"channel_name": channel_name, "password": hashed_password, "username": self.username})
        if response["status"] == "success":
            messagebox.showinfo("Join Channel", f"Rejoindre le canal {channel_name} avec succès!")
            self.root.destroy()
            chat_app = ChatApp(self.username, channel_name, response)
            chat_app.run()
        else:
            messagebox.showerror("Error", "Nom de canal ou mot de passe incorrect.")

    def create_channel(self):
        channel_name = self.channel_entry.get()
        password = self.password_entry.get()
        hashed_password = hash_password(password)
        response = self.send_request("create_channel", {"channel_name": channel_name, "password": hashed_password,"username":self.username})
        if response["status"] == "success":
            messagebox.showinfo("Create Channel", f"Canal {channel_name} créé avec succès!")
        else:
...
class ChatApp:
    ...
    def gestion(self):
        ...
        channel_label = tk.Label(new_window, text="Nom de l'utilisateur :", font=('helvetica', 15, 'bold'), bg='#74abdc',fg='#cb2e30')
        channel_label.pack(pady=(50,0))
        ...
        admin_check = tk.Checkbutton(new_window, text="Admin", font=('helvetica', 10), variable=self.admin_var, bg='#74abdc')
        admin_check.pack()
        reader_check = tk.Checkbutton(new_window, text="Lecteur", font=('helvetica', 10), variable=self.reader_var, bg='#74abdc')
        reader_check.pack()
        editor_check = tk.Checkbutton(new_window, text="Editeur", font=('helvetica', 10), variable=self.editor_var, bg='#74abdc')
        ...
        join_button = tk.Button(button_frame, text="Ajouter", command=self.ajouter_users, font=('helvetica', '10', 'bold'))
        join_button.pack(side=tk.LEFT, padx=5, pady=10)
        create_button = tk.Button(button_frame, text="Supprimer", command=self.supprimer_users, font=('helvetica', '10', 'bold'))
        create_button.pack(side=tk.LEFT, padx=5, pady=10)
        return_button = tk.Button(new_window, text="Retourner", command=self.return_users, font=('helvetica', '10', 'bold'))
    ...
    def ajouter_users(self):
        ...
        self.socket.sendall(json.dumps({"action": "add_users", "data": {"username":username, "role": roles, 'channel': self.channel}}).encode())
    ...
    def supprimer_users(self):
        ...
        self.socket.sendall(json.dumps({"action": "erase_users", "data": {"username":username,"channel":self.channel}}).encode())
    ...
    def connect(self):
        ...
        try:
            ...
            self.socket.sendall(json.dumps({"action": "join_channel", "data": {"channel_name": self.channel, "username": self.username}}).encode())
        except Exception as e:
    ...
    def send_message(self):
        ...
        if message:
            try:
                self.socket.sendall(json.dumps({"action": "send_message", "data": {"channel_name": self.channel, "message": message, "username": self.username}}).encode())
    ...
    def update_messages(self):
        ...
        while True:
            time.sleep(2)
            try:
                self.socket.sendall(json.dumps({"action": "get_new_messages", "data": {"channel_name": self.channel, "last_message_id": self.last_message_id}}).encode())
                ...
                break
                messagebox.showerror("Update Error", f"Unable to update messages: {e}")
...
class FileApp():
    def __init__(self,username,channel,role):
        ...
        if self.role==1:
            sender_button = tk.Button(self.root, text="Send File", width=20, height=2, command=self.launch_sender)
            sender_button.pack(pady=10)
        receiver_button = tk.Button(self.root, text="Receive Files", width=20, height=2, command=self.launch_receiver)
        receiver_button.pack(pady=10)

        exit_button = tk.Button(self.root, text="Exit", width=20, height=2, command=self.exit)
